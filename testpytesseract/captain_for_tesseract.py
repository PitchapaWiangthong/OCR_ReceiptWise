...
for index in range(160,220,5):
    directory_folder_source=r'testpytesseract\dataset\scb\captain_slip_test'
    images=[]
    ocr_result = []
    filename_list=[]
    for filename in os.listdir(directory_folder_source):
        if os.path.isfile(os.path.join(directory_folder_source, filename)):
            ...
            # image = image[250:1600]
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            
            #binarize
            thresh=cv2.threshold(gray,index,255,cv2.THRESH_BINARY_INV)[1]
            cv2.imwrite(f"testpytesseract\captain_iterate_asset\\threshold\scb_thresh210_{filename}{index}.png",thresh)
    ...
    contact_sheet.save(f'testpytesseract\\0{index}contact_sheet_thresh.png')
    contact_sheet.show() 