...
output_folder_ocr_text = r'testpytesseract\OCR\kbank\ocr_text'
...
for filename in os.listdir(directory_folder_source):
    if os.path.isfile(os.path.join(directory_folder_source, filename)):
        ...
        cv2.imwrite(f"testpytesseract\OCR\kbank\\threshold_image\\kbank_thresh150_{filename}.png", thresh)
...
with open('testpytesseract\OCR\kbank\ocr.txt', 'w', encoding='utf-8') as file:
